...
# binary.
RUN mkdir src/
RUN echo 'fn main() {println!("Wrong main!!")}' > ./src/main.rs
RUN source $HOME/.cargo/env && \
    if [ "$FEATURES" == "" ]; then \
    cargo build --release \
    --target="$RUST_TARGET"; \
...
RUN rm -f target/$RUST_TARGET/release/deps/keymashr*
COPY ./src ./src
...
RUN source $HOME/.cargo/env && \
    ...
    cp target/$RUST_TARGET/release/keymashr /keymashr && \
    actual-strip /keymashr

FROM alpine:latest

COPY --from=build /keymashr /keymashr

CMD ["./keymashr", "discord"]
